use chaar::{
    dtype::Dtype,
    graph::{graph_group::GraphGroup, nodes::Operator},
...
fn main() {
    ...
    let c = graph_group.add_operator(graph, Operator::Add, vec![a, b], stringify!(c).to_string());

    let d = graph_group.add_operator(graph, Operator::Add, vec![c, a], stringify!(d).to_string());

    let d = graph_group.add_operator(graph, Operator::Add, vec![d, b], stringify!(e).to_string());
